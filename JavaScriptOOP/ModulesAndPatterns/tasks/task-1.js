...
function solve() {
	var Course = {
		...
		addStudent: function (name) {
			var student = {};
			var studentNames = validateStudentName(name);
	...
	function validateTitle(title) {
		...
		if (typeof title === 'string') {
			if (title[0] === ' ' ||
				...
				title.indexOf('  ') > 0 ||
				title === '') {
				throw new Error('InvalidTitleException!');
			}
		} else {
			if (len === 0) {
				throw new Error('NoCoursePresentationsException!');
			}
		...
		function checkName(input) {
			...
			for (i = 1; i < len; i += 1) {
				if ((input.charCodeAt(i) < 97 || input.charCodeAt(i) > 122)) {
					throw new Error('LowercaseLettersException!(Names must be lowercase only!)');
				}
			}
		}

		if (namesArr.length > 2) {
			throw new Error('StudentNamesException!(Students cannot have more than 2 names!)');
		}

		if ((firstName.charCodeAt(0) < 65 || firstName.charCodeAt(0) > 90) ||
			(lastName.charCodeAt(0) < 65 || lastName.charCodeAt(0) > 90)) {
			throw new Error('UppercaseLetterException!(Names must start with capital letter only!)');
		}
	...
	function validateHomeworkID(id, presentationsArr) {
		if (id < presentationsArr.length || id > presentationsArr.length) {
			throw new Error('InvalidIdPresentationException!');
		}
	}

	function validateStudentID(id, studentsArr) {
		if (id < 1) {
			throw new Error('InvalidStudentIdException!');
		}
